    ...
    public class RechargeInfoService : IRechargeInfoService
    {
        ...
        public async Task<string> AirtimeTopup(string phoneNumber, double amount)
        {
            ...
            if(user != null)
            {
                ...
                transactionHistory.Id = Guid.NewGuid();
                transactionHistory.UserId = user.Id;
                transactionHistory.Amount = amount;
                transactionHistory.TransactionDescription = "Airtime Topup";
                transactionHistory.TypeofTransaction = "Credit";
                transactionHistory.IsSuccessful = isTopupSuccessful;
                transactionHistory.Date = DateTime.Now;

                _unitOfWork.GetRepository<TransactionHistory>().Insert(transactionHistory);
                await _unitOfWork.SaveChangesAsync();
        ...
        public async Task<string> TransferAirtimeTopup(string phoneNumber, double amount) 
        {
            ...
            if (user != null) 
            {
                ...
                transactionHistory.UserId = user.Id;
                transactionHistory.Amount = amount;
                transactionHistory.TransactionDescription = "Transfer Airtime Topup";
                transactionHistory.TypeofTransaction = "Credit";
                transactionHistory.IsSuccessful = isTopupSuccessful;
                transactionHistory.Date = DateTime.Now;
